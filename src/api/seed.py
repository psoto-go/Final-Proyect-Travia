from api.models import db, User, Hotel, HotelArchives, Room, Reviews, Service, Services, HotelArchives, Booking, City, RoomArchives
...
class SeedData:
    ...
    def create_hotel(self):
        ...
        self.hotel4 = Hotel(name = "ibis budget Huerta De Murcia", description = "El ibis budget Huerta De Murcia está ubicado en Murcia, a 3 km del puente romano y a 3,8 km del Museo de Bellas Artes de Murcia. Este hotel de 1 estrella tiene salón común y habitaciones con aire acondicionado, WiFi gratuita y baño privado. y aparcamiento privado.Este hotel sirve un desayuno a la carta. El ibis budget Huerta De Murcia alberga una terraza solárium. El personal de la recepción 24 horas habla inglés y francés. Se encuentra a 2,4 km del palacio de San Esteban y a 4 km de la plaza de toros de Murcia. El aeropuerto internacional Región de Murcia, situado a 22 km del ibis budget Huerta De Murcia, es el más cercano", longitude = "38.00176468377343", latitude = "-1.1485858843290098", city_id = self.city.id, favorite = False )
        self.hotel5 = Hotel(name = "B&B Hotel Murcia", description = "El B&B Hotel Murcia se encuentra en Murcia, a 6 km del Museo de Bellas Artes de Murcia y del puente romano. Este hotel de 2 estrellas cuenta con salón compartido y habitaciones con aire acondicionado, conexión WiFi gratuita y baño privado. El alojamiento dispone de recepción 24 horas y organiza excursiones. Todas las habitaciones del hotel tienen escritorio y TV de pantalla plana. También incluyen armario. El B&B Hotel Murcia sirve un desayuno continental. La plaza de toros de Murcia se encuentra a 6 km del alojamiento, mientras que el Campus de Espinardo - Universidad de Murcia está a 2,5 km. El aeropuerto internacional de la Región de Murcia es el más cercano y está a 25 km del B&B Hotel Murcia.", longitude = "38.031784226841374", latitude = " -1.1513772817329826", city_id = self.city.id, favorite = False )
        
        self.hotel6 = Hotel(name = "TRYP Murcia Rincón de Pepe", description = "El TRYP Murcia Rincón de Pepe se encuentra en el centro de Murcia, a solo 100 metros de la catedral de Murcia y junto al nuevo Mercado de Correos. Ofrece centro de fitness y habitaciones con aire acondicionado y WiFi gratuita. Las habitaciones son luminosas, están insonorizadas y presentan una decoración moderna. Disponen de TV vía satélite de pantalla plana, minibar y baño privado con bañera, secador de pelo y artículos de aseo gratuitos. El hotel alberga 2 restaurantes, el Rincón de Pepe, de estilo clásico, y La Barra del Rincón, que es más informal, sirve tapas españolas y cuenta con una zona de comedor al aire libre. Todas las mañanas se sirve un desayuno buffet. El bar de cócteles La Muralla está situado en la planta baja y cuenta con paredes árabes del siglo XVI. En los alrededores del hotel hay una gran variedad de bares y sitios para comer. El establecimiento se halla a 15 minutos de la estación de tren de Murcia El TRYP Murcia Rincón de Pepe se encuentra a 10 minutos a pie del palacio de San Esteban.", longitude = "39.574555694206595", latitude = "2.64210587648824", city_id = self.city1.id, favorite = False )
        ...
        db.session.add(self.hotel5)
        db.session.add(self.hotel6)
        db.session.commit()
    ...
    def create_room(self):
        ...
        self.room4 = Room(kind = "matrimonio", number_of_beds = 1, number_of_persons = 2, hotel_id = self.hotel4.id, price = 30)
        self.room4 = Room(kind = "individual", number_of_beds = 1, number_of_persons = 1, hotel_id = self.hotel4.id, price = 50)
        self.room5 = Room(kind = "suite", number_of_beds = 1, number_of_persons = 2, hotel_id = self.hotel2.id, price = 40)
        self.room6 = Room(kind = "matrimonio", number_of_beds = 1, number_of_persons = 2, hotel_id = self.hotel5.id, price = 30)
        self.room7 = Room(kind = "suite", number_of_beds = 1, number_of_persons = 2, hotel_id = self.hotel5.id, price = 40)
        self.room8 = Room(kind = "doble", number_of_beds = 2, number_of_persons = 2, hotel_id = self.hotel6.id, price = 50)
        self.room9 = Room(kind = "individual", number_of_beds = 1, number_of_persons = 1, hotel_id = self.hotel6.id, price = 50)
        ...
        db.session.add(self.room6)
        db.session.add(self.room7)
        db.session.add(self.room8)
        db.session.commit()
    ...
    def create_review(self):
        ...
        self.review7 = Reviews(hotel_id = self.hotel4.id, user_id = self.user.id, description = "Todo en general, sobre todo destacó el equipo humano, Cristina la chica de recepción nos aconsejó desde primera hora y nos trato estupendo, Juan el chico de la piscina súper buena gente y carlos no podía faltar increíbles cócteles y sobre todo como persona un 10. Habitaciones y zonas comunes limpias, vistas inmejorables. Volveremos seguro, un saludo de los Sevillanos.", characteristic = "limpio, piscina")
        self.review8 = Reviews(hotel_id = self.hotel4.id, user_id = self.paco.id, description = "El hotel es bastante nuevo, la habitación perfecta para nosotros estando con un bebé. Baño muy cómodo y bien reformado. Cama muy cómoda. Muy limpio. Bien de amenities.", characteristic = "limpio, comodo", favorite = False)
        self.review9 = Reviews(hotel_id = self.hotel5.id, user_id = self.user.id, description = "Todo en general, sobre todo destacó el equipo humano, Cristina la chica de recepción nos aconsejó desde primera hora y nos trato estupendo, Juan el chico de la piscina súper buena gente y carlos no podía faltar increíbles cócteles y sobre todo como persona un 10. Habitaciones y zonas comunes limpias, vistas inmejorables. Volveremos seguro, un saludo de los Sevillanos.", characteristic = "limpio, piscina")
        self.review10 = Reviews(hotel_id = self.hotel5.id, user_id = self.paco.id, description = "El hotel es bastante nuevo, la habitación perfecta para nosotros estando con un bebé. Baño muy cómodo y bien reformado. Cama muy cómoda. Muy limpio. Bien de amenities.", characteristic = "limpio, comodo", favorite = False)
        self.review11 = Reviews(hotel_id = self.hotel6.id, user_id = self.user.id, description = "Todo en general, sobre todo destacó el equipo humano, Cristina la chica de recepción nos aconsejó desde primera hora y nos trato estupendo, Juan el chico de la piscina súper buena gente y carlos no podía faltar increíbles cócteles y sobre todo como persona un 10. Habitaciones y zonas comunes limpias, vistas inmejorables. Volveremos seguro, un saludo de los Sevillanos.", characteristic = "limpio, piscina")
        self.review12 = Reviews(hotel_id = self.hotel6.id, user_id = self.paco.id, description = "El hotel es bastante nuevo, la habitación perfecta para nosotros estando con un bebé. Baño muy cómodo y bien reformado. Cama muy cómoda. Muy limpio. Bien de amenities.", characteristic = "limpio, comodo", favorite = False)
        ...
        db.session.add(self.review8)
        db.session.add(self.review9)
        db.session.add(self.review10)
        db.session.add(self.review11)
        db.session.add(self.review12)
    ...
    def create_services(self):
        ...
        self.create_services49 = Services(hotel_id=self.hotel4.id, service_id = self.service1.id)
        self.create_services50= Services(hotel_id=self.hotel4.id, service_id = self.service2.id)
        self.create_services51 = Services(hotel_id=self.hotel4.id, service_id = self.service3.id)
        self.create_services52= Services(hotel_id=self.hotel4.id, service_id = self.service4.id)
        self.create_services53= Services(hotel_id=self.hotel4.id, service_id = self.service5.id)
        self.create_services54= Services(hotel_id=self.hotel4.id, service_id = self.service6.id)
        self.create_services55= Services(hotel_id=self.hotel4.id, service_id = self.service7.id)
        self.create_services56= Services(hotel_id=self.hotel4.id, service_id = self.service8.id)
        self.create_services57= Services(hotel_id=self.hotel4.id, service_id = self.service9.id)
        self.create_services58 = Services(hotel_id=self.hotel4.id, service_id = self.service10.id)
        self.create_services59 = Services(hotel_id=self.hotel4.id, service_id = self.service11.id)
        self.create_services60 = Services(hotel_id=self.hotel4.id, service_id = self.service12.id)

        self.create_services61 = Services(hotel_id=self.hotel5.id, service_id = self.service1.id)
        self.create_services62= Services(hotel_id=self.hotel5.id, service_id = self.service2.id)
        self.create_services63 = Services(hotel_id=self.hotel5.id, service_id = self.service3.id)
        self.create_services64= Services(hotel_id=self.hotel5.id, service_id = self.service4.id)
        self.create_services65= Services(hotel_id=self.hotel5.id, service_id = self.service5.id)
        self.create_services66= Services(hotel_id=self.hotel5.id, service_id = self.service6.id)
        self.create_services67= Services(hotel_id=self.hotel5.id, service_id = self.service7.id)
        self.create_services68= Services(hotel_id=self.hotel5.id, service_id = self.service8.id)
        self.create_services69= Services(hotel_id=self.hotel5.id, service_id = self.service9.id)
        self.create_services70 = Services(hotel_id=self.hotel5.id, service_id = self.service10.id)
        self.create_services71 = Services(hotel_id=self.hotel5.id, service_id = self.service11.id)
        self.create_services72 = Services(hotel_id=self.hotel5.id, service_id = self.service12.id)

        self.create_services73 = Services(hotel_id=self.hotel6.id, service_id = self.service1.id)
        self.create_services74= Services(hotel_id=self.hotel6.id, service_id = self.service2.id)
        self.create_services75 = Services(hotel_id=self.hotel6.id, service_id = self.service3.id)
        self.create_services76= Services(hotel_id=self.hotel6.id, service_id = self.service4.id)
        self.create_services77= Services(hotel_id=self.hotel6.id, service_id = self.service5.id)
        self.create_services78= Services(hotel_id=self.hotel6.id, service_id = self.service6.id)
        self.create_services79= Services(hotel_id=self.hotel6.id, service_id = self.service7.id)
        self.create_services80= Services(hotel_id=self.hotel6.id, service_id = self.service8.id)
        self.create_services81= Services(hotel_id=self.hotel6.id, service_id = self.service9.id)
        self.create_services82 = Services(hotel_id=self.hotel6.id, service_id = self.service10.id)
        self.create_services83 = Services(hotel_id=self.hotel6.id, service_id = self.service11.id)
        self.create_services84 = Services(hotel_id=self.hotel6.id, service_id = self.service12.id)
        ...
        db.session.add(self.create_services49)
        db.session.add(self.create_services50)
        db.session.add(self.create_services51)
        db.session.add(self.create_services52)
        db.session.add(self.create_services53)
        db.session.add(self.create_services54)
        db.session.add(self.create_services55)
        db.session.add(self.create_services56)
        db.session.add(self.create_services57)
        db.session.add(self.create_services58)
        db.session.add(self.create_services59)
        db.session.add(self.create_services60)
        db.session.add(self.create_services61)
        db.session.add(self.create_services62)
        db.session.add(self.create_services63)
        db.session.add(self.create_services64)
        db.session.add(self.create_services65)
        db.session.add(self.create_services66)
        db.session.add(self.create_services67)
        db.session.add(self.create_services68)
        db.session.add(self.create_services69)
        db.session.add(self.create_services70)
        db.session.add(self.create_services71)
        db.session.add(self.create_services72)
        db.session.add(self.create_services73)
        db.session.add(self.create_services74)
        db.session.add(self.create_services75)
        db.session.add(self.create_services76)
        db.session.add(self.create_services77)
        db.session.add(self.create_services78)
        db.session.add(self.create_services79)
        db.session.add(self.create_services80)
        db.session.add(self.create_services81)
        db.session.add(self.create_services82)
        db.session.add(self.create_services83)
        db.session.add(self.create_services84)
    ...
    def create_archives(self):
        ...
        self.create_archives17 = HotelArchives(hotel_id = self.hotel4.id, url = "https://res.cloudinary.com/pabletesglez/image/upload/v1630512266/277895163_svgvoz.jpg")
        self.create_archives18 = HotelArchives(hotel_id = self.hotel4.id, url = "https://res.cloudinary.com/pabletesglez/image/upload/v1630512334/277895205_y6lx2z.jpg")
        self.create_archives19 = HotelArchives(hotel_id = self.hotel4.id, url = "https://res.cloudinary.com/pabletesglez/image/upload/v1629284944/q_sj5rqw.jpg")
        self.create_archives20 = HotelArchives(hotel_id = self.hotel4.id, url = "https://res.cloudinary.com/pabletesglez/image/upload/v1629282561/b1_h8yvh9.jpg")
        self.create_roomarchives9 = RoomArchives(room_id = self.room4.id, url = "https://res.cloudinary.com/pabletesglez/image/upload/v1629284944/qqq_vfemxu.jpg")
        self.create_roomarchives10 = RoomArchives(room_id = self.room4.id, url = "https://res.cloudinary.com/pabletesglez/image/upload/v1629283882/h1_z4lxvy.jpg")

        self.create_archives21 = HotelArchives(hotel_id = self.hotel5.id, url = "https://res.cloudinary.com/pabletesglez/image/upload/v1630512720/297507677_jjhdgs.jpg")
        self.create_archives22 = HotelArchives(hotel_id = self.hotel5.id, url = "https://res.cloudinary.com/pabletesglez/image/upload/v1630512720/297507709_aucq4c.jpg")
        self.create_archives23 = HotelArchives(hotel_id = self.hotel5.id, url = "https://res.cloudinary.com/pabletesglez/image/upload/v1630512720/297507587_ejyban.jpg")
        self.create_archives24 = HotelArchives(hotel_id = self.hotel5.id, url = "https://res.cloudinary.com/pabletesglez/image/upload/v1630512720/297507612_enymbu.jpg")
        self.create_roomarchives11 = RoomArchives(room_id = self.room5.id, url = "https://res.cloudinary.com/pabletesglez/image/upload/v1629284944/qqq_vfemxu.jpg")
        self.create_roomarchives12 = RoomArchives(room_id = self.room5.id, url = "https://res.cloudinary.com/pabletesglez/image/upload/v1629283882/h1_z4lxvy.jpg")

        self.create_archives25 = HotelArchives(hotel_id = self.hotel6.id, url = "https://res.cloudinary.com/pabletesglez/image/upload/v1630512720/297507677_jjhdgs.jpg")
        self.create_archives26 = HotelArchives(hotel_id = self.hotel6.id, url = "https://res.cloudinary.com/pabletesglez/image/upload/v1630512720/297507709_aucq4c.jpg")
        self.create_archives27 = HotelArchives(hotel_id = self.hotel6.id, url = "https://res.cloudinary.com/pabletesglez/image/upload/v1630512720/297507587_ejyban.jpg")
        self.create_archives28 = HotelArchives(hotel_id = self.hotel6.id, url = "https://res.cloudinary.com/pabletesglez/image/upload/v1630512720/297507612_enymbu.jpg")
        self.create_roomarchives13 = RoomArchives(room_id = self.room6.id, url = "https://res.cloudinary.com/pabletesglez/image/upload/v1629284524/1111_ggiyuf.jpg")
        self.create_roomarchives14 = RoomArchives(room_id = self.room6.id, url = "https://res.cloudinary.com/pabletesglez/image/upload/v1629284524/1111_ggiyuf.jpg")
        self.create_roomarchives15 = RoomArchives(room_id = self.room7.id, url = "https://res.cloudinary.com/pabletesglez/image/upload/v1629284524/1111_ggiyuf.jpg")
        self.create_roomarchives16 = RoomArchives(room_id = self.room7.id, url = "https://res.cloudinary.com/pabletesglez/image/upload/v1629284524/1111_ggiyuf.jpg")
        self.create_roomarchives17 = RoomArchives(room_id = self.room8.id, url = "https://res.cloudinary.com/pabletesglez/image/upload/v1629284524/1111_ggiyuf.jpg")
        self.create_roomarchives18 = RoomArchives(room_id = self.room8.id, url = "https://res.cloudinary.com/pabletesglez/image/upload/v1629284524/1111_ggiyuf.jpg")
        ...
        db.session.add(self.create_archives17)
        db.session.add(self.create_archives18)
        db.session.add(self.create_archives19)
        db.session.add(self.create_archives20)
        db.session.add(self.create_archives21)
        db.session.add(self.create_archives22)
        db.session.add(self.create_archives23)
        db.session.add(self.create_archives24)
        db.session.add(self.create_archives25)
        db.session.add(self.create_archives26)
        db.session.add(self.create_archives27)
        db.session.add(self.create_archives28)
        ...
        db.session.add(self.create_roomarchives9)
        db.session.add(self.create_roomarchives10)
        db.session.add(self.create_roomarchives11)
        db.session.add(self.create_roomarchives12)
        db.session.add(self.create_roomarchives13)
        db.session.add(self.create_roomarchives14)
        db.session.add(self.create_roomarchives15)
        db.session.add(self.create_roomarchives16)
        db.session.add(self.create_roomarchives17)        
        db.session.commit()
